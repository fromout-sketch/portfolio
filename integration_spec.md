...
1. **Вычитывание сообщения**: Адаптер вычитывает сообщение из Kafka.
2. **Проверка условий**: Проверяет условие `order_event_type_name = ORDER_STATUS_TRANSITION` и `order_status_name = PASSED_FOR_EXECUTION`.
3. **Сохранение в БД**: Создает запись заказа в своей БД.
4. **Маппинг данных**: Выполняет маппинг полей сообщения на структуру API Достависта (см. раздел 7).
5. **Отправка запроса**: Отправляет POST-запрос на создание заказа в Достависту (`/api/business/1.5/create-order`).
...
### 4.2 Сценарий
1. **Приём сообщения**: Адаптер получает сообщение с `order_status_name = CANCELLED_BY_CLIENT`.
2. **Обновление БД**: Обновляет статус заказа в своей БД (`order_status = canceled`).
3. **Особенность процесса**: 
   * **Важно**: В данной версии адаптер не отправляет команду отмены в Достависту.
   * **Зона ответственности**: Этот процесс полностью делегирован на OMS.
   * **Механика**: OMS самостоятельно отправляет запрос на отмену в Достависту, используя `system_contractor_order_id`.
   * **Роль адаптера**: Синхронизировать локальный статус.
...
###  5.1 Предусловие
1. **Событие в Достависте**: В системе Достависта происходит изменение статуса заказа или доставки.
2. **Отправка уведомления**: Достависта отправляет веб-хук (POST) на Callback URL адаптера.


### 5.2 Сценарий обработки статусов доставки
### 5.2 Сценарий обработки статусов доставки
1. **Приём веб-хука**: Адаптер получает веб-хук о статусе доставки (`delivery`).
2. **Логирование**: Записывает полученные данные в историю доставок (`delivery_history`).
3. **Маппинг статуса**: На основе статуса доставки (например, `courier_assigned`, `parcel_picked_up`, `finished`) формирует соответствующий статус для OMS (`ROUTE_ASSIGNED`, `CARGO_PICKED_UP`, `CARGO_DELIVERED`).
4. **Финансовый блок**: Если требуется, запрашивает у Достависты детализацию стоимости (для расчета компенсаций).
5. **Отправка в OMS**: Отправляет итоговое событие в OMS через REST API (`/v2.0/tasks/{id}/events`).

### 5.3 Сценарий обработки статусов заказа (Отмены)
1. **Приём веб-хука**: Адаптер получает веб-хук о статусе заказа (`order`) со статусом `canceled`.
2. **Определение типа отмены**: Определяет тип отмены по полю `status_description`:
   * **Canceled via API** -> Маппится в `CANCELLED_BY_CLIENT`. Дополнительно устанавливает флаг `is_payment_returnable` на основе истории статусов.
   * **Все остальные причины** -> Маппится в `CANCELLED_BY_CONTRACTOR`. Также маппит `status_description` на внутренний код причины отмены.
   * **Специальные статусы** (Груз утерян, Заказ не был выполнен) -> Маппятся на `CARGO_LOST_BY_CONTRACTOR` и `EXECUTION_FAILED`.
3. **Отправка в OMS**: Отправляет итоговое событие в OMS.
...
| Статус Достависты | Статус OMS | Примечание |
| :--- | :--- | :--- |
...
| canceled | CANCELLED_BY_CLIENT | По инициативе клиента |
| canceled | CANCELLED_BY_CONTRACTOR / EXECUTION_FAILED / CARGO_LOST_BY_CONTRACTOR | По инициативе исполнителя |
...
### 6.2 Сценарий
1. **Получение запроса**: `МП ПР` -> `Тарификатор` -> `Адаптер`: Получение запроса на расчет (`POST /dostavista/v1.0/tariff`).
2. **Логирование**: Адаптер создает запись расчета в своей БД.
3. **Маппинг и запрос**: Выполняет маппинг полей запроса и отправляет запрос на расчет в Достависту (`POST /calculate-order`).
4. **Фиксация результата**: Получает результат, сохраняет в БД и возвращает в Тарификатор.
...
| Поле из Webhook Достависта | Поле для OMS | Примечание |
| :--- | :--- | :--- |
...
* **`order`** — основная таблица заказов:
  ...
  * `client_order_id` — идентификатор задачи из OMS (`task_id`).
  * `system_contractor_order_id` — идентификатор заказа в системе Достависта (`order_id`).
  * `order_status` — текущий статус заказа.
  * `subject_data` — информация о грузе / содержание заказа.
  * `delivery_id` — идентификатор доставки.
  * `pickup / delivery данные` — блок полей для адресов и контактных данных сторон.

* **`delivery_history`** — история изменений статусов доставок:
  * `delivery_id` — идентификатор доставки (Foreign Key).
  * `client_order_id` — связующий идентификатор задачи из OMS.
  * `status` — зафиксированный статус на момент события.
  * `event_datetime` — дата и время изменения статуса.

* **`retry_tab_oms` / `retry_tab_dostavista`** — служебные таблицы для очередей:
  * Хранение сообщений для повторных попыток отправки в случае сбоев.

### 8.4 ERD модель
``` plantuml
@startuml
' Настройка стилей для ТЗ
hide circle
...
entity "order" as order {
  * **id** : UUID [PK]
  --
  * **client_order_id** : VARCHAR [UK] -- task_id из OMS
  system_contractor_order_id : VARCHAR -- order_id из Достависты
...
entity "delivery_history" as dh {
  * **id** : BIGINT [PK]
  --
  * **delivery_id** : VARCHAR [FK]
  * **client_order_id** : VARCHAR [FK]
  status : VARCHAR
  event_datetime : TIMESTAMP
}

entity "retry_tab_oms" as oms_retry {
  * **id** : BIGINT [PK]
  --
  * **client_order_id** : VARCHAR [FK]
  payload : TEXT
...
entity "retry_tab_dostavista" as dv_retry {
  * **id** : BIGINT [PK]
  --
  * **client_order_id** : VARCHAR [FK]
  payload : TEXT
...
order ||--o{ dh : "содержит"
order ||--o{ oms_retry : "логирует сбои"
order ||--o{ dv_retry : "логирует сбои"

@enduml
``` 

### 8.3 Справочник причин отмены (OMS)
Код	Описание
